...
try:
    ...
    try:
        ...
        print("[WARNING] Pip not installed on system")
...
def redraw_gameWindow(win, bo, p1, p2, color, ready):
    ...
    if not color == "s":
        ...
        if bo.turn == color:
            txt3 = font.render(bo.p1Name + "\'s TURN", 1, (255, 0, 0))
            win.blit(txt3, (width / 2 - txt3.get_width() / 2, 700))
        else:
            txt3 = font.render(bo.p2Name + "\'s TURN", 1, (255, 0, 0))
            win.blit(txt3, (width / 2 - txt3.get_width() / 2, 700))
...
pygame.display.set_caption(name+"\'s Chess Game")
